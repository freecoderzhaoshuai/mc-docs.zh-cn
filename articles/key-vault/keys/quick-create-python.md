...
ms.openlocfilehash: 70dc321686ff340eba36d6b9c3d8437b34dfbf09
ms.sourcegitcommit: 39410f3ed7bdeafa1099ba5e9ec314b4255766df
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/16/2020
ms.locfileid: "90678364"
---
...
[API 参考文档](https://docs.microsoft.com/python/api/overview/azure/keyvault-keys-readme?view=azure-python) | [库源代码](https://github.com/Azure/azure-sdk-for-python/tree/master/sdk/keyvault/azure-keyvault-keys) | [包（Python 包索引）](https://pypi.org/project/azure-keyvault-keys/)
...
运行以下 [az keyvault set-policy](/cli/keyvault?view=azure-cli-latest#az-keyvault-set-policy) 命令，授权服务主体对密钥进行删除、获取、列出和创建操作。 
...
在前面的代码中，[`DefaultAzureCredential`](https://docs.microsoft.com/python/api/azure-identity/azure.identity.defaultazurecredential?view=azure-python) 对象使用针对服务主体创建的环境变量。 每当从 Azure 库创建客户端对象（例如 [`KeyClient`](https://docs.microsoft.com/python/api/azure-keyvault-keys/azure.keyvault.keys.keyclient?view=azure-python)）以及要通过该客户端使用的资源的 URI 时，都要提供此凭据：
...
获取密钥保管库的客户端对象后，可以使用 [create_rsa_key](https://docs.microsoft.com/python/api/azure-keyvault-keys/azure.keyvault.keys.keyclient?view=azure-python#create-rsa-key-name----kwargs-) 方法存储密钥： 

```python
rsa_key = client.create_rsa_key(keyName, size=2048)
```

还可以使用 [create_key](https://docs.microsoft.com/python/api/azure-keyvault-keys/azure.keyvault.keys.keyclient?view=azure-python#create-key-name--key-type----kwargs-) 或 [create_ec_key](https://docs.microsoft.com/python/api/azure-keyvault-keys/azure.keyvault.keys.keyclient?view=azure-python#create-ec-key-name----kwargs-)。
...
若要读取 Key Vault 中的密钥，请使用 [get_key](https://docs.microsoft.com/python/api/azure-keyvault-keys/azure.keyvault.keys.keyclient?view=azure-python#get-key-name--version-none----kwargs-) 方法：

```python
retrieved_key = client.get_key(keyName)
 ```

还可以使用 Azure CLI 命令 [az keyvault key show](/cli/keyvault/key?view=azure-cli-latest#az-keyvault-key-show) 来验证是否设置了密钥。

### <a name="delete-a-key"></a>删除密钥

若要删除密钥，请使用 [begin_delete_key](https://docs.microsoft.com/python/api/azure-keyvault-keys/azure.keyvault.keys.keyclient?view=azure-python#begin-delete-key-name----kwargs-) 方法：
...
可以使用 Azure CLI 命令 [az keyvault key show](/cli/keyvault/key?view=azure-cli-latest#az-keyvault-key-show) 来验证是否已删除密钥。
