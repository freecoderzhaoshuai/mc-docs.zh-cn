...
ms.date: 08/21/2020
ms.author: v-tawe
ms.openlocfilehash: 5ee65fc2ec8e5229424f2cda26034f54d84283d5
ms.sourcegitcommit: 2e9b16f155455cd5f0641234cfcb304a568765a9
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 08/21/2020
ms.locfileid: "88715337"
---
...
在 Azure SignalR 服务资源的“概览”边栏选项卡中，我们已经为你选择了合适的聚合类型。 如果转到“指标”边栏选项卡，则可以将[此处](../azure-monitor/platform/metrics-supported.md#microsoftsignalrservicesignalr)的聚合类型用作参考。
...
模式：
* `Default` 模式需要中心服务器。 当没有可用于中心的服务器连接时，客户端连接到该中心的尝试会失败。
* `Serverless` 模式不允许任何服务器连接，即它将拒绝所有服务器连接，所有客户端必须处于无服务器模式。
* `Classic` 模式为混合状态。 当中心具有服务器连接时，新客户端会被路由到中心服务器；否则，客户端会进入无服务器模式。

  这可能会导致某些问题，例如，所有服务器连接暂时丢失，某些客户端会进入无服务器模式，而不是路由到中心服务器。

选择方案：
1. 如果没有中心服务器，请选择 `Serverless`。
1. 如果所有中心都有中心服务器，请选择 `Default`。
...
使用 Azure SignalR 时，ASP.NET SignalR 的某些 API 和功能不再受支持：
...
对 ASP.NET SignalR 的支持侧重于兼容性，因此并非 ASP.NET Core SignalR 的所有新功能都受支持。 例如，MessagePack 和流式处理等仅适用于 ASP.NET Core SignalR 应用程序 。

可以将 SignalR 服务配置为不同的服务模式：`Classic`/`Default`/`Serverles`。 在此 ASP.NET 支持中，不支持 `Serverless` 模式。 也不支持数据平面 REST API。

## <a name="where-do-my-data-reside"></a>我的数据存放在什么位置？

Azure SignalR 服务以数据处理器服务的形式运行。 该服务不会存储任何客户内容，并且设计会保证数据驻留。 如果将 Azure SignalR 服务与其他 Azure 服务（例如用于诊断的 Azure 存储）一起使用，请参阅[此文](https://azure.microsoft.com/resources/achieving-compliant-data-residency-and-security-with-azure/)，了解如何将数据驻留在 Azure 区域中。
