...
指标是 Azure 资源发出的并由 Azure Monitor 服务使用的 Azure 遥测数据（也称为性能计数器）。 Batch 帐户中的指标示例包括“池创建事件”和“任务完成事件”。

<!--Not Available on Low-Priority Node Count, -->
...
4. 为指标选择“聚合”选项。 对于基于计数的指标（如“专用核心计数”），请使用“平均”聚合。 对于基于事件的指标（如“池重设大小完成事件数”），请使用“计数”聚合。
    
    <!--Not Available on  or "Low-Priority Node Count"-->
...
还可以使用 Azure Monitor [REST API](https://docs.microsoft.com/rest/api/monitor/) 配置准实时警报。 有关详细信息，请参阅 [Azure 中的警报概述](../azure-monitor/platform/alerts-overview.md)。 若要在警报中包含特定于作业、任务或池的信息，请参阅[借助 Azure Monitor 警报对事件做出响应](../azure-monitor/learn/tutorial-response.md)中有关搜索查询的信息。
...
以下是 `PT1H.json` 日志文件中 `PoolResizeCompleteEvent` 条目的示例。 它包括有关专用的当前和目标数量以及操作的开始和结束时间的信息：

<!--Not Availble on and low-priority nodes-->
